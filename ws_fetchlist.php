	...
	if(isset($_POST['student_id'])){
		...
		if(mysql_num_rows($result)>0){
			...
			while($row = mysql_fetch_array($result))
				$jsonarray[]=array("id"=>$row["id"],"student_id"=>$row["student_id"],"student_name"=>$row1["name"],"hostel_id"=>$row["hostel_id"],"hostel_no"=>$row["hostel_no"],"room"=>$row["room"],"bed"=>$row["bed"],"address"=>$row["address"],"fromdate"=>$row["fromdate"],"tilldate"=>$row["tilldate"],"purpose_of_leave"=>$row["purpose_of_leave"]);
